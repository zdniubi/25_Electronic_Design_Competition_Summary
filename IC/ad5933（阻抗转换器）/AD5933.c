...
uint8_t data_clk = 0x00;  // 外:0x80 内0x00  时钟选择
...
uint8_t data_StartM = 0x00; // 1kHz=0x007d02,50kHz=ox186A73,35kHz=0x11174D,10kHz=0x04E217
...
void AD5933_ReadImpedance(ImpeType *AA)
{
	uint8_t j = 0;
	...
	AA->Amplitude = sqrt((float)re*re + (float)im*im);
	AA->Impedance = (float)(amp*10029/sqrt((float)re*re + (float)im*im)); 
	AA->Phase = atan2(im,re) - 1.913;
}
...
void AD5933_FreInit(float Fre, float AddFre) // 启动一个频率点的扫描
{
	...
	AD5933_WriteByte(0x84, data_StartL); // 100kHz=0x30D4E8
...
void AD5933_WriteByte(uint8_t RegOrIns, uint8_t DataOrReg) // 写一个字节到寄存器 或者指令
{
	...
	//	IIC_Stop();
	HAL_I2C_Mem_Write(&hi2c4, D_ADDR, RegOrIns, 1, &DataOrReg, 1, 100);
}
...
uint8_t AD5933_ReadByte(void) // 读一个字节
{
	...
	//	IIC_Stop();
	HAL_I2C_Master_Receive(&hi2c4, D_ADDR | 0x01, &Temp, 1, 100);
	return (Temp);
}
